...
sf_use_s2(FALSE)
in_situ_shp <- st_read("Shapefiles/In Situ Oil Sands Scheme Approvals/In Situ Oil Sands Scheme Approval_GCS_NAD83.shp")%>%clean_names()%>%
  st_transform(crs=st_crs(gd_pts))
  
  in_situ <- st_filter(in_situ_shp, blcn_terr, .predicate = st_intersects)%>%
    #filter(sch_status!="SU")%>%
    filter(sch_status=="AC")%>%
    filter(sch_sub_ty %in% c("Commercial-SAGD","Commercial-CSS"))
  
  in_situ_oth<-st_filter(in_situ_shp, blcn_terr, .predicate = st_intersects)%>%
...
#in_situ<-in_situ %>% left_join(aer_active_schemes)  
...
st37_blcn<- st_intersection(st37_shp,blcn_buffer_20)

st37_dh_blcn<- st_intersection(st37_dh,blcn_buffer_20)
...
ggplot(st37_blcn%>%filter(relevant,fluid_short_description %in% c("GAS","CR-BIT","CR-OIL")|type_short_description=="SAGD")) +
  ...
  geom_sf(data=res_pts,aes(colour="Beaver Lake Cree Nation Reserve"),fill=NA,linewidth=1) +
    #geom_sf(data = larp_pop%>%filter(OBJECTID %in% c(3,6,7)))+
  scale_color_manual("",values=c("dodgerblue","red","darkgreen"))
