...
#os_data<-os_data%>%select(project_name,mine,big_project,reporting_year,production=cleaned_crude_bitumen_at_rcp_barrels,gross_revenue,capital_costs,operating_costs,other_net_proceeds,other_costs,royalty_payable,payout_status,last_prod)
#save(os_data,file = "os_data.rdata")
#save(os_data,file="../../Courses/econ_366_w26/files/os_data.rdata")
```
...
ggplot(os_data %>% filter(mine,!is.infinite(op_profit))%>%
         mutate(project_type=ifelse(mine,"Mine","In Situ"),
                project_name_factor=as_factor(project_name),
                project_name_factor=fct_reorder(project_name_factor,last_prod),
                payout=as_factor(payout_status),
                payout=fct_relevel(payout,"PRE")
                ),
